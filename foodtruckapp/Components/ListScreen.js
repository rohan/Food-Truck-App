...
export default function ListScreen({ navigation }) {
        ...
        get(child(dbRef, `users`)).then((dataSnapshot) => {
          if (dataSnapshot.exists()) {
            ...
            for (var j = 0; j < users.length; j++) {
              ...
              const sortedTruckData = trucks.sort((a, b) =>
  ...
  function Item({ item }) {
    ...
    return (
      <TouchableOpacity
        style={styles.item}
        onPress={() => navigation.navigate("Description", { truck: item })}
      >
        {truckToImageMap.get(item.name) && (
          <Image source={truckToImageMap.get(item.name)} style={styles.image} />
        )}
        <View style={styles.itemContent}>
          <Text style={styles.itemText}>{item.name}</Text>
          <Text style={styles.locationText}>{item.location}</Text>
          <Text style={styles.timeText}>
            {getTime(item.startTime)} to {getTime(item.endTime)}
          </Text>
        </View>
